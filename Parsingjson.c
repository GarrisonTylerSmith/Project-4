...
Scene read_scene(char* json_name){
  ...
  while(1){
    ...
    float set_theta = 0;
    float theta = 0;
    float set_reflectivity = 0;
    float reflectivity = 0;
    float set_refractivity = 0;
    ...
    if(objtype == T_SPHERE){
      ...
      if(reflectivity < 0){
        fprintf(stderr, "Sphere must have a non-negative reflectivity! Line %d\n", line);
        exit(1);
      }
      if(refractivity < 0){
        fprintf(stderr, "Sphere must have a non-negative refractivity! Line %d\n", line);
        exit(1);
      }
      if(ior < 0){
        fprintf(stderr, "Sphere must have a non-negative ior! Line %d\n", line);
        exit(1);
      }
